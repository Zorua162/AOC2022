...
func move_crates(amount int, from int, to int, storage [][]string) [][]string {
	...
	end_of_array := len(storage[from-1])
	fmt.Println(end_of_array-(amount), end_of_array)
	fmt.Println(storage, storage[from-1])
	stack_move := storage[from-1][end_of_array-(amount) : end_of_array]
	fmt.Println(stack_move)
	storage[to-1] = append(storage[to-1], stack_move...)
	storage[from-1] = storage[from-1][:len(storage[from-1])-amount]
	// }
	return storage
}
